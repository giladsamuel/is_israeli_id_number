# This program check is a given ID is valid number by the "check digit" system
# 
# @author: Gilad Samuel.
# @version: 1.0.0
...
if len(ID) != 9 :                                        # ID don't have 9 digits
    ...
    ID = int(ID)                                          # change ID varibel to be int from str

    while ID > 0:                                         # put all digits of ID in a list
    ...
    for x in range(9):                                    # multiply ID digits with check_list digits
        ...
        if temp <=9:                                      # if result of multiply greater then 9, sum of digits indted
            check_list[x] = temp                          
        else:
